...
function hasSecGroups {
    ...
    if [ ${#groups} -eq 0 ]; then
        ...
        parts=($groups)
        if [ ${#parts[@]} -ne $expectedCount ]; then
...
function assignAllFloatingIPs {
    for x in `nova list | awk '{if ($2 != "ID" && NF>1 && $13 == "|") print $2}'`; do
        port=`neutron port-list -- --device_id $x | awk '{if ($2 != "id" && NF>1) print $2}'`;
